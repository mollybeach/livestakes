    ...
    <div className="relative">
      ...
      >
        ...
        {stream_url && (
          <button
            onClick={toggleMute}
            className="absolute top-2 right-2 sm:top-3 sm:right-3 w-6 h-6 sm:w-8 sm:h-8 bg-black bg-opacity-60 hover:bg-opacity-80 rounded-full flex items-center justify-center text-white transition-all duration-200"
          >
            {isMuted ? (
              <svg
      ...
      <div className="p-3 sm:p-4">
        ...
        <div className="flex gap-2">
          ...
          >
            <span>🎯</span>
            <span className="hidden sm:inline">Bet on Project</span>
            <span className="sm:hidden">Bet</span>
          </Button>
          {id && (
            <button
              onClick={() => setShowMarketAssociation(true)}
              className="px-2 sm:px-3 py-2 sm:py-2.5 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-lg transition-colors text-xs sm:text-sm"
